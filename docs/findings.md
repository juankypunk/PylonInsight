...
## F-013 — BatteryView text export encoding bug

**Status:** ❌ Eliminado

**Evidence**

* Reproduced on SC0500A BMS.
* Reproduced on legacy BMU.
* Reproduced on XHB BMU.
* Reproduced in event.txt.
* Reproduced in event_detailed.txt.
* event.csv exports are not affected.

## Impact

The text exports cannot be considered reliable input for automated parsing without encoding detection and recovery.
CSV exports remain the preferred source for structured data.

## F-013 — Text export encoding

**Status:** ✅ Confirmed

**Description:** All text exports generated by BatteryView 3.9.4 are encoded using UTF-16 Little Endian (UTF-16LE).

**Affected files:**

- history.txt
- history_detailed.txt
- event.txt
- event_detailed.txt

**Evidence**

Verified using the Linux `file` utility.

**Example output:**

Unicode text, UTF-16, little-endian text

**Implications:** Parsers must explicitly read text exports using UTF-16 encoding.
